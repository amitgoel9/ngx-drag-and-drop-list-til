    ...
    public handleDrop(event: DragEvent): boolean {
        ...
        // Insert the object into the array, unless dnd-drop took care of that (returned true).
           if (data !== true) {
					...
					var dataIndex = this.dndModel.findIndex(i => i.comment_id === data.comment_id);

					var insertionPoint = 0;
					var deletionIndex = -1;
					if (dataIndex < index) {
						insertionPoint = index - offset;
						deletionIndex = dataIndex;
					} else {
						insertionPoint = index;
						deletionIndex = dataIndex+1;
					}

				this.dndModel.splice(insertionPoint, 0, data);

				this.dndModel.splice(deletionIndex,1);
				}
